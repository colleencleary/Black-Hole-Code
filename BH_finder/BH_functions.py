...
def do_all(snap_id, filename='/data/scratch/jillian/h229/h229.cosmo50PLK.3072g3HbwK1BH.00%s/h229.cosmo50PLK.3072g3HbwK1BH.00%s'):
...
def plot_all(iord_data, iord_df, time, iords, h, save=False, filename='plots/i%s_plots.png'):
    iord=int(np.unique(iord_df['iord']))
    h_id=int(iord_data['halo'][-1:])
    #plt.xlim(0,14)
    
    f,(ax1, ax2, ax3, ax4, ax5)=plt.subplots(5, sharex=True, dpi=100)
    ax1.plot(time['time[Gyr]'], iord_data['t_mass'], label='Total Mass [M$_\odot$]', linewidth=.9, color='k')
    ax1.plot(time['time[Gyr]'], iord_data['g_mass'], linestyle='--', label='Gas Mass [M$_\odot$]', linewidth=.8, color='#1B7837')
    ax1.plot(time['time[Gyr]'], iord_data['s_mass'], linestyle='--', label='Star Mass [M$_\odot$]', linewidth=.9, color='#FFEE33')
    ax1.plot(time['time[Gyr]'], iord_data['bh_mass'], linestyle='--', label='BH Mass', linewidth=.8, color='#662506')
    ax2.plot(iord_df['adj_time'], iord_df['mass'], linewidth=.8, linestyle='--', label='BH Mass [M$_\odot$]', color='#662506');
    ax3.plot(time['time[Gyr]'], iord_data['r[kpc]'], label='BH Radial Distance [kpc]', color='#88CCEE');
    ax5.plot(iord_df['adj_time'], iord_df['x_lum'], linewidth=.6, color='#6059A9', label='X-ray Luminosity [ergs$ s$^{-1}$]')
    sfhist,thebins=SFH(h[h_id].s, massform=False, subplot=ax4, color='#44AA99', label='SFR [M$_\odot$ yr$^{-1}$]')

    ax1.set_title('Properties of  i%s ($z$=0) Throughout Time'%(iord), y=1.4,weight='bold')
    ...
    time_axis=pd.read_csv('../h148/csvdata/time_axis.csv',index_col=0)
